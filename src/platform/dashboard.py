...
class DashboardPlatform:
    ...
    async def execute_pending_effects(self, kernel: Kernel) -> PlatformRunResult:
        receipts = 0
        for record in await kernel.claim_effect_requests(frozenset({DASHBOARD_REPLY_CAPABILITY})):
            ...
            try:
                ...
                receipts += 1
                logger.info(
                    "dashboard effect succeeded record_id=%s episode_id=%s request_id=%s",
                    record.record_id,
                    record.episode_id,
                    request_id,
                )
                ...
                logger.log(
                    logging.ERROR,
                    "dashboard effect failed record_id=%s episode_id=%s request_id=%s error_type=%s",
                    record.record_id,
                    record.episode_id,
                    request_value,
                    type(error).__name__,
