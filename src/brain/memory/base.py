...
class MemoryContext:
    ...
    def to_prompt_text(self) -> str:
        ...
        if self.working_context:
            ...
            prompt_lines.extend(f"- {item.role}: {item.content}" for item in self.working_context)
            prompt_lines.append("")  # 空行分隔

        # 2. 拼接 L2 情景记忆 (最近发生的系统事件/日志)
        if self.episodic_events:
            prompt_lines.append("[相关历史事件]")
            prompt_lines.extend(f"- {event}" for event in self.episodic_events)
            prompt_lines.append("")

        # 3. 拼接 L3 语义记忆 (长期事实、用户偏好)
        if self.semantic_facts:
            prompt_lines.append("[已知事实与偏好]")
            prompt_lines.extend(f"- {fact}" for fact in self.semantic_facts)
            prompt_lines.append("")
