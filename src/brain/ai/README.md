...
| 角色         | 用途                           | 默认模型                     | 环境变量                       |
| ------------ | ------------------------------ | ---------------------------- | ------------------------------ |
| `fast`       | 简单对话 / 门控判定            | `deepseek/deepseek-chat`     | `LLM_GATEWAY_FAST_MODEL`       |
| `quality`    | 复杂推理 / 记忆压缩 / 动作规划 | `deepseek/deepseek-reasoner` | `LLM_GATEWAY_QUALITY_MODEL`    |
| `multimodal` | 图片理解                       | `deepseek/deepseek-chat`     | `LLM_GATEWAY_MULTIMODAL_MODEL` |
| `embedding`  | 文本嵌入                       | `BAAI/bge-m3`                | `LLM_GATEWAY_EMBEDDING_MODEL`  |
...
```python
from src.brain.ai.gateway import gateway, CancelledWithPartialResponse
...
#     "by_model": {"deepseek/deepseek-chat": {"count": 10, "cost": 0.001}, ...},
#     "records": [...]
# }
```
...
# {"fast": "deepseek/deepseek-chat", "quality": "deepseek/deepseek-reasoner", ...}
```

## 环境变量

```ini
# fast 角色模型（默认: deepseek/deepseek-chat）
LLM_GATEWAY_FAST_MODEL=deepseek/deepseek-chat

# quality 角色模型（默认: deepseek/deepseek-reasoner）
LLM_GATEWAY_QUALITY_MODEL=deepseek/deepseek-reasoner

# multimodal 角色模型（默认: deepseek/deepseek-chat）
LLM_GATEWAY_MULTIMODAL_MODEL=deepseek/deepseek-chat

# embedding 角色模型（默认: BAAI/bge-m3）
