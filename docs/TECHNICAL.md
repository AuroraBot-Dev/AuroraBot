...
> 持久化机制、工具域划分与演化指南。**持续与实现保持同步**（对应 RFC 0210 后的现状）。
>
> 设计基准：`docs/rfc/`（0200-0210）；本文档与已接受 RFC 冲突时以 RFC 为准。
...
| 工具完成语义 | `complete_task=True` 且成功 | **存储层直接完成 agent**（不投递消息，RFC 0203） |
...
| `triage.py` | `TriageBatch`/`InboxEvent`/`TriageLimits`（决策类型已删，RFC 0209） |
...
**模式**：启动时一次加载 → 不可变快照 → `get()` 零参数获取；变更靠重启生效（RFC 0206）。
...
**模式**：逻辑同构代码化——`BaseAgent` 基类（RFC 0210）。
...
**Agent 三元组**（RFC 0209）：`AgentProfile`（agents.toml）= ① implementation（逻辑类）② capabilities（权限域）③ model_role（模型角色）；上下文由 `handle_claim` 构造。
...
**模式**：单一 `MemoryService`（memory.sqlite3）既是被动投影的目标，也是主动工具的存储（同源，RFC 0207）。
...
READY ──claim──▶ (处理中，无持久化中间态)
  ...
  ├── failure ──▶ FAILED
  ├── 任务被终止 ──▶ CANCELLED
  └── 其他决策 ──▶ READY（等待语义由 activities/children 派生，RFC 0205）
```
...
| RFC | 主题 | 现状 |
| --- | --- | --- |
| 0200 | Agent 中心运行时、包边界 | 基准 |
| 0201 | 有界上下文与运行态留存 | 部分被 0210 取代（JSON 归档/JSONL） |
| 0202 | Triage/Root/压缩记忆基线 | TriagePolicy 与 admit 被 0209 取代；防抖/三层存储/留存仍有效 |
| 0203 | Agent 自由与运行时边界 | 有效（complete_task 语义） |
| 0204 | Console 本地前端 | 有效 |
| 0205 | 状态机收敛、决策链路扁平化 | 有效（派生等待状态） |
| 0206 | 运行时组合与配置快照 | 有效 |
| 0207 | Multiagent 委派链与同源记忆 | 注意力形态被 0209 取代；记忆同源有效 |
| 0208 | 层级重定性 | 有效（五层角色） |
| 0209 | Agent 全同构 | 有效（入口 triage agent、委派唯一化） |
| 0210 | 最小 engine 重写 | 有效（单一 SQLite、无租约、BaseAgent、v9） |
