...
状态：已接受
日期：2026-07-24
来源：取代 RFC 0100、0102、0106、0107 中关于 Kernel、localhost、Platform 依赖方向与运行时组合的决定
...
- 检查路径由 `src.localhost` 提供。localhost 负责输入规范化、命令路由、状态检查和调试接口，不参与 engine pump。
...
| `src.localhost` | 命令、检查、调试与输入分发 | 所有 `src` 实现包 |
| `aurora` | 唯一进程组合根 | 所有下层 |
...
- Platform 不导入 localhost 或 engine；交互输入和 AMP 入口通过 contracts Port 注入。
- engine 不导入 ai、memory、agents、prompt、config、platform 或 localhost。
- localhost 不被 engine、Platform 或其他热路径实现依赖。
- `src` 不导入 `aurora`。
...
localhost 仅持有 engine 的公开操作与查询端口，用于 `/status`、`/pump`、`/task`、`/agent`、普通会话输入及调试投影。engine 主循环不调用 localhost。

## 配置与迁移

结构配置按 `ARCHITECTURE.md` 的包级 TOML 拆分。迁移必须保持未知键启动前失败、profile 仅覆盖 runtime、密钥仅来自显式命名环境变量，并将运行数据迁移到 `data/engine`。不提供旧工作区的隐式兼容读取。

源代码迁移按以下顺序进行：共享 contracts、engine 热路径、localhost sidecar、aurora 组合根、配置与存储路径。每一步均由导入边界和行为测试约束。
